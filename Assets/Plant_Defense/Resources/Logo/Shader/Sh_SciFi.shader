	...
	Subshader
	{
		...
		Pass
		{
			...
			half4 frag(vertexOutput i , float facing : VFACE) : COLOR
			{
				...
				float4 HDR_factor = float4(1,1,1,0) * 0.3 ;
			
				float4 outliner_col = tex2D(_OutlineTex ,i.outliner)* temp *(_Outline_Opacity) * _Outline_Color * HDR_factor;

				float4 bf_col = tex2D(_BackFace_Diffuse_map,i.uv_matcap);

				//COLORBOOST
				outliner_col.rgb *= dot(ff_col.rgb,float3(0.5,0.5,0.5));
				ff_col.rgb *= _FrontFace_Intensity * _FrontFace_Color;
				bf_col.rgb *= _BackFace_Intensity * _BackFace_Color;
				ff_col.a = _FrontFace_Color.a ;
				bf_col.a = _BackFace_Color.a ;

				// Add Outline To the Color
				bf_col = bf_col + (outliner_col);
